...
export default class FilePad {
  ...
  async initialize (filePath) {
    ...
    if(filePath && fs.existsSync(filePath)) {
      try {
        ...
        this.reps = (await (new Blocks(filePath, 'rep')).extractBlocks()).filter(b => b.from === b.to) // just inlines
      } catch(e) {
        console.error(e)
  ...
  getRepByName (lookupName) {
    return this.reps.find(rep => {
      const { name } = this.parseRep(rep)
      return name === lookupName
    })
  }
  
  getRep (ln) {
    return this.reps.find(rep => (ln === rep.from && ln === rep.to))
  }

  isRepWithin (ln) {
    return this.getRep(ln) !== undefined
  }
  ...
  parseRep(rep) {
    const tokens = rep.name.split(',')
    if(tokens.length < 3) {
      throw Error(`Broken rep block name (${rep.name}). It should be in the format // rep name, /regexp/, value`)
    }
    const name = tokens[0].trim()
    const r = tokens[1].trim()
    const regexp = regexParser(r)
    const value = tokens[2].trim()
    return { name, r, regexp, value}
  }
  
  executeRep(ln) {
    const repBlock = this.getRep(ln)
    const { name, r, regexp, value } = this.parseRep(repBlock)
    const remoteRep = this.basePad.getRepByName(name)
    const { regexp: remoteRegexp } = this.parseRep(remoteRep)
    return `${repBlock.content.replace(remoteRegexp, value)}${this.delimiters.inline} rep ${name},${r},${value}`
  }
  ...
  processLine(l, ln) {
    ...
    if (stamp) {
      ...
      // TODO: split res by \n and process like each line, to support rep inline block
    ...
    if(ln === 18) {
      console.log('ASK FOR REP!!!', { l, ln, repsAre: this.reps })
    }
    if (this.isRepWithin(ln)) {
      return `${this.executeRep(ln)}\n`
    }
  ...
  updateWith(basePad) {
    ...
    this.reps = basePad.reps.map(baseRep => {
      const { name: baseName } = this.parseRep(baseRep)
      const targetRep = this.getRepByName(baseName)
      if (!targetRep) {
        return Object.assign({}, baseRep)
      }
      return Object.assign({}, targetRep, { from: baseRep.from, to: baseRep.to })
    })
